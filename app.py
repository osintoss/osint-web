    ...
    if request.method == 'POST':
        ...
        if query:
            ...
            if search_type == 'photo':
                # Поиск фотографий по открытым базам и соцсетям
                results = [
                    {"title": "Яндекс Картинки (Точный поиск по ФИО)", "url": f"https://yandex.ua/images/search?text=%22{encoded_query}%22", "status": "Yandex"},
                    {"title": "Google Images (Точное совпадение лица/ФИО)", "url": f"https://www.google.com/search?q=%22{encoded_query}%22&tbm=isch", "status": "Google"},
                    {"title": "Фотографии в ВКонтакте (VK)", "url": f"https://vk.com/search?c%5Bsection%5D=auto&c%5Bq%5D={encoded_query}", "status": "VK"},
                    {"title": "Bing Visual & Photo Search", "url": f"https://www.bing.com/images/search?q=%22{encoded_query}%22", "status": "Bing"}
                ]
